...
const gameOverSound = new Audio('music/move2.wav');
...
function gameEngine() {
    ...
    if (isCollide(snakeArr)) {
        ...
        musicSound.pause();
        inputDir = { x: 0, y: 0 };
        alert("Game Over! refresh and play again");
        gameOverSound.play();
        snakeArr = [{ x: 13, y: 15 }];
        musicSound.play();
        score = 0;
    ...
    if (snakeArr[0].y === food.y && snakeArr[0].x === food.x) {
        ...
        if (score > Highscoreval) {
            Highscoreval = score;
            localStorage.setItem("highscor", JSON.stringify(Highscoreval));
